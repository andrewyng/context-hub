...
// Create a VPC for Batch compute environments
const vpc = new ec2.Vpc(this, 'BatchVpc', { maxAzs: 2 });

// --- Fargate compute environment + job definition + queue ---
const fargateComputeEnv = new batch.FargateComputeEnvironment(this, 'FargateComputeEnv', {
  vpc,
  spot: true,
});

const fargateJobRole = new iam.Role(this, 'FargateJobRole', {
  assumedBy: new iam.ServicePrincipal('ecs-tasks.amazonaws.com'),
});

const fargateJobDef = new batch.EcsJobDefinition(this, 'FargateJobDef', {
  jobDefinitionName: 'MyFancyBatchJob',
  container: new batch.EcsFargateContainerDefinition(this, 'FargateContainer', {
    image: ecs.ContainerImage.fromRegistry('public.ecr.aws/amazonlinux/amazonlinux:latest'),
    // cpu is specified in CPU units (1024 = 1 vCPU). Here we reserve 2 vCPU.
    cpu: 2048,
    memory: Size.mebibytes(1024),
    jobRole: fargateJobRole,
  }),
});

const fargateQueue = new batch.JobQueue(this, 'FargateJobQueue', {
  computeEnvironments: [{ computeEnvironment: fargateComputeEnv, order: 1 }],
});

// --- Managed EC2 (ECS) compute environment + job definition + queue ---
const managedEc2ComputeEnv = new batch.ManagedEc2EcsComputeEnvironment(this, 'ManagedEc2CE', {
  vpc,
  // instanceTypes, min/max vCPUs and other options are available in props
  // instanceTypes: [new ec2.InstanceType('t3.medium')],
});

const ec2JobRole = new iam.Role(this, 'Ec2JobRole', {
  assumedBy: new iam.ServicePrincipal('ecs-tasks.amazonaws.com'),
});

const ec2JobDef = new batch.EcsJobDefinition(this, 'Ec2JobDef', {
  // Define the container using the EC2 container definition class
  container: new batch.EcsEc2ContainerDefinition(this, 'Ec2Container', {
    image: ecs.ContainerImage.fromRegistry('amazon/amazon-ecs-sample'),
    // Reserve 1 vCPU (1024 CPU units)
    cpu: 1024,
...
const ec2Queue = new batch.JobQueue(this, 'Ec2JobQueue', {
  computeEnvironments: [{ computeEnvironment: managedEc2ComputeEnv, order: 1 }],
});
```

### CloudWatch Alarms and Monitoring

Use CloudWatch Alarms to monitor resources and trigger notifications or auto-scaling actions.
...
// DynamoDB throttle alarm using built-in metric
...
dashboard.addWidgets(
  new cloudwatch.GraphWidget({
    ...
    left: [listFn.metricInvocations(), listFn.metricErrors()],
    width: 12,
  }),
  new cloudwatch.GraphWidget({
    title: 'Lambda Duration',
    left: [listFn.metricDuration({ statistic: 'p50' }), listFn.metricDuration({ statistic: 'p99' })],
    width: 12,
  }),
);
```
...
const busKey = new kms.Key(this, 'BusKey', {
  enableKeyRotation: true,
});

const bus = new events.EventBus(this, 'AppBus', {
  eventBusName: 'MyAppBus',
  description: 'Event Bus dedicated to my application',
  kmsKey: busKey,
  logConfig: {
    includeDetail: events.IncludeDetail.FULL,
    level: events.Level.TRACE,
  },
});

const rule = new events.Rule(this, 'OrderProcessingRule', {
...
// Scheduled rule with cron
const cronRule = new events.Rule(this, 'DailyProcessing', {
...
cronRule.addTarget(new targets.LambdaFunction(processFn));
```
...
topic.addSubscription(
  new subscriptions.LambdaSubscription(processFn, {
    filterPolicy: {
...
topic.grantPublish(processFn);

// SNS→SQS Fan-Out pattern example
...
const ordersQueue = new sqs.Queue(this, 'OrdersQueue', {
  queueName: 'orders',
  deadLetterQueue: { queue: topicDlq, maxReceiveCount: 5 },
...
const analyticsQueue = new sqs.Queue(this, 'AnalyticsQueue', {
  ...
  deadLetterQueue: { queue: topicDlq, maxReceiveCount: 10 },
  encryption: sqs.QueueEncryption.SQS_MANAGED,
  retentionPeriod: Duration.days(7),
});

// Subscribe queues with filter policies for selective fan-out
topic.addSubscription(new subscriptions.SqsSubscription(ordersQueue, {
  filterPolicy: {
    eventType: sns.SubscriptionFilter.stringFilter({ allowlist: ['order.created', 'order.updated'] }),
  },
...
topic.addSubscription(new subscriptions.SqsSubscription(analyticsQueue, {
  filterPolicy: {
    eventType: sns.SubscriptionFilter.stringFilter({ allowlist: ['order.*'] }),
  },
  rawMessageDelivery: true,
}));
```
...
// Grant Lambda permission to consume
queue.grantConsumeMessages(processFn);

// Add Lambda as event source
