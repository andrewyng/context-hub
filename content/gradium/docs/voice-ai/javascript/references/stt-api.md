...
const turnEnded = msg.vad[2].inactivity_prob > 0.5;
```
...
async function sttWithVad(audioData, apiKey) {
    return new Promise((resolve, reject) => {
        ...
        ws.on("message", (data) => {
            ...
            if (msg.type === "ready") {
                const frameSize = msg.frame_size; // 1920
                const chunkBytes = frameSize * 2; // 16-bit = 2 bytes/sample

                // Send audio in chunks
                for (let i = 0; i < audioData.length; i += chunkBytes) {
                    const chunk = audioData.slice(i, i + chunkBytes);
                    ws.send(JSON.stringify({
                        type: "audio",
                ...
                console.log(`Transcript: ${msg.text} (at ${msg.start_s}s)`);
                transcripts.push(msg.text);
            } else if (msg.type === "step") {
                const vadProb = msg.vad[2].inactivity_prob;
                if (vadProb > 0.5) {
                    console.log(`Turn ended (prob=${vadProb.toFixed(2)})`);
                }
