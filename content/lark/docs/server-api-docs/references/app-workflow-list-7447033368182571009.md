...
Source URL: https://open.feishu.cn/document/docs/bitable-v1/app-workflow/list
Updated: 2026-02-11
...
支持的应用类型 | 自建应用、商店应用
权限要求<br>**调用该 API 所需的权限。开启其中任意一项权限即可调用**<br>开启任一权限即可 | 查看自动化流程/工作流(base:workflow:read)<br>查看、评论、编辑和管理多维表格(bitable:app)<br>查看、评论和导出多维表格(bitable:app:readonly)
